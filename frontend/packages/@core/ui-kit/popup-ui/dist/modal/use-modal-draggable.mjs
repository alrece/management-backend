...
//#region src/modal/use-modal-draggable.ts
function useModalDraggable(targetRef, dragRef, draggable, containerSelector, centered, overflow) {
	...
	const onDraggable = () => {
		...
		if (dragDom && targetRef.value) dragDom.addEventListener("mousedown", onMousedown);
	};
	const offDraggable = () => {
		const dragDom = unrefElement(dragRef);
		if (dragDom && targetRef.value) dragDom.removeEventListener("mousedown", onMousedown);
	};
	const resetPosition = () => {
		transform.offsetX = 0;
		transform.offsetY = 0;
